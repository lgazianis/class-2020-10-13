...
```{r sc1}
random_beads <- tibble(replicate = 1:5) %>%
...
```{r sc2}
beads_50 <- tibble(replicate = 1:1000) %>%
  mutate(samp50 = map(replicate, ~sample_n(urn, size = 50, replace = TRUE))) %>%
  mutate(count_red = map_int(samp50, ~sum(.$color == "red"))) %>%
  mutate(count_blue = map_int(samp50, ~sum(.$color == "blue"))) %>%
  mutate(more_red = if_else(count_red > count_blue, TRUE, FALSE)) %>%
  summarize(prop_true = sum(more_red == TRUE)/n())
```
